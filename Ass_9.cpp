...
int main()
{
    ...
    while (pc >= 0 && pc < max_pc)
    {
        ...
        if (ex_mem_reg.id != "nop")
        {
            ...
            string x = ex_mem_reg.id;

            ////forwarding unit
            if ((x == "add" || x == "sub" || x == "sll" || x == "srl" || x == "lw") && ex_mem_reg.write_reg != 0 && ex_mem_reg.write_reg == id_ex_reg.read_reg_1)
            {
                // forward_A = ex_mem_reg.result;
                id_ex_reg.read_value_1 = ex_mem_reg.result;
            }

            else if ((x == "add" || x == "sub" || x == "sll" || x == "srl" || x == "lw") && ex_mem_reg.write_reg != 0 && ex_mem_reg.write_reg == id_ex_reg.read_reg_2)
            {
